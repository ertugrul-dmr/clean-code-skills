...
description: Use when writing, fixing, editing, reviewing, or refactoring any Dart or Flutter code. Enforces Robert Martin's complete Clean Code catalog—naming, functions, comments, DRY, and boundary conditions.
---

# Clean Dart: Complete Reference

Enforces all Clean Code principles from Robert C. Martin's Chapter 17, adapted for Dart.
...
## Environment (E1-E2)
- E1: One command to build (`dart pub get` / `flutter pub get`)
- E2: One command to test (`dart test` / `flutter test`)

## Functions (F1-F4)
- F1: Maximum 3 arguments (use a class or named-parameter record for more)
...
- G24: Follow conventions (Effective Dart + `dart analyze`)
...
## Dart-Specific (D1-D3)
These adapt the Java-specific rules (J1-J3) to Dart conventions:
- D1: Keep imports explicit; prefer `show` on barrel imports to document dependencies and avoid name collisions
- D2: Use enums, not magic constants — prefer `enum Status { active, inactive }` over `const int statusActive = 1`
- D3: Type all public APIs explicitly; avoid `dynamic` at boundaries (Dart's sound type system makes this free)
...
| `dynamic` in public API | Specific types or generics |
